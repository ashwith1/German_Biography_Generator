...
def allowed_file(filename):
    return filename.lower().endswith(('.csv', '.docx'))
    ...
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            if file and allowed_file(file.filename):
                ...
                # Process the file
                output_pdf_path = process_file_and_update_status(file_path)
                ...
                flash("Invalid file format. Only CSV or DOCX files are allowed.", "danger")
...
def handle_start_processing(file_name):
    socketio.start_background_task(process_file_and_update_status, file_name)
