...
# together_api_key = "0c7ca1d8efd796b972cf1a5593343213bd4ab46451a2cecbcac333fa6f02793f"
    ...
    def generate_biography(self, input_text):
        ...
        prompt = """
I Generate a biography of an interviewee, including all provided information and answering the following questions in THIRD-PERSON in German. The biography should be written in a coherent, chronological order with a focus on ensuring that all significant life events, dates, relationships, and achievements are presented in detail. The biography must be 600-700 words, and each stage of the interviewee's life should be thoroughly covered as per the given questions.
Ensure the biography is written from a third-person perspective, not first-person. It should be presented as an autobiographical sketch in paragraphs, covering their birth and early family life, education, career and professional life, personal milestones, and other key life events. Do not lose any details from the questions, and incorporate the following aspects:
...
# Generate a biography of an interviewee, including all provided information and answering the following questions in THIRD-PERSON. The biography should be written in a coherent, chronological order with a focus on ensuring that all significant life events, dates, relationships, and achievements are presented in detail. The biography must be 600-700 words, and each stage of the interviewee's life should be thoroughly covered as per the given questions.
# Ensure the biography is written from a third-person perspective, not first-person. It should be presented as an autobiographical sketch in paragraphs, covering their birth and early family life, education, career and professional life, personal milestones, and other key life events. Do not lose any details from the questions, and incorporate the following aspects:
...
# Please ensure the biography is coherent, chronological, detailed, and presents a well-rounded view of the interviewee's life journey with years mentioned. Include years not mentioned in the biography previously.
...
def process_file_and_update_status(file):
    try:
        ...
        if transcript_data:
            socketio.emit('update', {'file': file_name, 'status': 'Processing'}, namespace='/')
            ...
            socketio.emit('update', {'file': file_name, 'status': 'Completed'}, namespace='/')
        else:
            socketio.emit('update', {'file': file_name, 'status': 'Failed'}, namespace='/')
    except Exception as e:
...
def download_file(filename):
    processed_folder = os.path.join(os.getcwd(), PROCESSED_FOLDER)
